...
export default function IssuerConsole(){
  ...
  // org
  const verifiedOrgs = listOrgs().filter(o=>o.status==="verified");
  const [orgId,setOrgId] = useState("");
  const [org,setOrg] = useState(null);

  // org listesi değişirse ilkini seç
  useEffect(()=>{
    if(!orgId && verifiedOrgs[0]) setOrgId(verifiedOrgs[0].id);
  },[verifiedOrgs, orgId]);
  useEffect(()=>{ setOrg(orgId ? getOrg(orgId) : null); },[orgId]);
  ...
  const saveTpl = ()=>{
    setTplErr("");
    if(!org) return;
    const p = tryParse(tplBody);
    if(!p.ok){ setTplErr(p.error); return; }
    putTemplate(org.id, tplKey, { name: tplName, body: p.value });
    setFlash({tone:"ok", text:"Taslak kaydedildi."});
    setTimeout(()=>setFlash(null), 1200);
    setOrg(getOrg(org.id));
  };

  const delTpl = ()=>{
    if(!org) return;
    if(!confirm(`'${tplKey}' taslağını sil?`)) return;
    removeTemplate(org.id, tplKey);
    setOrg(getOrg(org.id));
    setTplKey("studentCard"); setTplName("StudentCard");
    setFlash({tone:"info", text:"Taslak silindi."});
    setTimeout(()=>setFlash(null), 1200);
  };
  ...
  const orgTemplates = Object.entries(org?.templates || {}); // [ [key, {name, body}], ... ]
      ...
      <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--panel)] p-6 shadow-sm relative">
        ...
        <div className="mt-4">
          ...
          <div className="relative">
            ...
            >
              {verifiedOrgs.map(o=><option key={o.id} value={o.id}>{o.name} ({o.domain})</option>)}
            </select>
            <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[color:var(--muted)]" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 9l6 6 6-6"/></svg>
          </div>
                ...
                {orgTemplates.map(([k, v])=>(
                  <button
                    ...
                    onClick={()=>{
                      ...
                      setTplKey(k);
                      setTplName(v?.name || k);
                      setTplBody(JSON.stringify(v?.body ?? {}, null, 2));
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {org?.templates?.[tplKey] && <button onClick={delTpl} className="text-xs px-2 py-1 rounded-lg border border-[color:var(--border)] bg-[color:var(--panel)] hover:bg-[color:var(--panel-2)]">Sil</button>}
                </div>
              </div>
